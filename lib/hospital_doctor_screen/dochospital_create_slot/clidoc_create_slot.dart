...
class HospitalBookAppointmentScreen extends StatefulWidget {
  const HospitalBookAppointmentScreen({
    super.key,
    required this.clinicDoctorId,
  });
  final int clinicDoctorId; // Example clinic doctor ID
  @override
  State<HospitalBookAppointmentScreen> createState() =>
      _HospitalBookAppointmentScreenState();
}

class _HospitalBookAppointmentScreenState
    extends State<HospitalBookAppointmentScreen> {
  ...
  Future<void> postAvailability(bool available) async {
    final url = Uri.parse(
      "https://417sptdw-8005.inc1.devtunnels.ms/userapp/hospital-doctor/$clinicDoctorId/availability/",
    );
  //  print(url);
  ...
  Future<void> getDoctorDetails() async {
    final url = Uri.parse(
      'https://417sptdw-8005.inc1.devtunnels.ms/userapp/view_hospital_doctor/$clinicDoctorId/',
    );
    print(url);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        leading: IconButton(onPressed: () => HospaitalDocHomeScreen(doctorId: clinicDoctorId!,), icon: Icon(Icons.arrow_back)),
          ...
          state.when(
            ...
            success: (response) {
              ...
              Navigator.of(context).push(
                MaterialPageRoute(
                  builder: (context) {
                    return HospaitalDocHomeScreen(doctorId: clinicDoctorId!);
                  },
                ),
              );
            },
                              ...
                              ? CircleAvatar(
                                  backgroundImage: NetworkImage(
                                    doctorDetails!['image']
                                            ...
                                            .startsWith('http')
                                        ? doctorDetails!['image']
                                        : 'https://417sptdw-8005.inc1.devtunnels.ms${doctorDetails!['image']}',
                                  ),
                                  radius: 20,
                                )
                              : CircleAvatar(
